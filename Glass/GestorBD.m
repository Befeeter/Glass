...
- (instancetype) initWithDatabaseFilename: (NSString *) dbFileName{ NSArray *paths = nil;
    self = [super init];
    if (self){
...
-(void) ejecutaConsulta: (const char*) consulta esEjecutable: (BOOL) consultaEjecutable
{
    sqlite3* baseDeDatos;
    NSString *caminoBD = [self.carpetaDocumentos stringByAppendingPathComponent:self.nombreBD]; sqlite3_stmt* consultaCompilada;
    NSMutableArray *arrayAux; char* datos;
    int numCols;
    //Inicializamos array de resultados
    if (self.arrResultados != nil)
    {
        [self.arrResultados removeAllObjects]; self.arrResultados = nil;
    }
    ...
    if(sqlite3_prepare_v2(baseDeDatos, consulta, -1, &consultaCompilada, NULL) != SQLITE_OK){
        NSLog(@"Errorrrr en la consulta :%s",sqlite3_errmsg(baseDeDatos));
        return;
    }
    ...
    if (!consultaEjecutable){ //Cargamos datos en un array auxiliar ...
        ...
        numCols = sqlite3_column_count(consultaCompilada);
            ...
            datos = (char*)sqlite3_column_text(consultaCompilada,i); if (datos != NULL){
                [arrayAux addObject:[NSString stringWithUTF8String:datos]];
            }
                if (self.arrNombresCols.count != numCols){
                    datos = (char*) sqlite3_column_name(consultaCompilada,i);
                    [self.arrNombresCols addObject:[NSString stringWithUTF8String:datos]];
                }
            }
            if (arrayAux.count > 0){
